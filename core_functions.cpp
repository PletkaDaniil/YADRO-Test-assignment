...
void process_club_work_and_handle_clients(const std::vector<client_info>& active_clients_data, std::map<int, bool>& table_availability, int start_of_work, int end_of_work, int table_price, int total_number_of_tables) {
    ...
    for (const auto& client: active_clients_data){
        switch (client.command){
            case 1:
                handle_client_event_1(client, clients_waiting_for_tables, clients_in_club, start_of_work, end_of_work);
                break;
            case 2:
                handle_client_event_2(client, table_availability, client_to_table, time_spent_at_each_table, table_price,
                                        start_of_work, end_of_work, clients_waiting_for_tables, clients_in_club);
                break;
            case 3:
                handle_client_event_3(client, table_availability, clients_waiting_for_tables, clients_in_club, total_number_of_tables);
                break;
            case 4:
                handle_client_event_4(client, table_availability, client_to_table, time_spent_at_each_table, table_price, end_of_work, clients_waiting_for_tables, clients_in_club);
                break;
        }
    }
...
void handle_client_event_1 (const client_info& client, std::queue<std::string>& clients_waiting_for_tables, std::set<std::string>& clients_in_club, int start_of_work, int end_of_work) {
...
void handle_client_event_2 (const client_info& client, std::map<int, bool>& table_availability, std::map<std::string, table_info>& client_to_table, std::map<int, result_after_work>& time_spent_at_each_table,
                            int table_price, int start_of_work, int end_of_work, std::queue<std::string>& clients_waiting_for_tables, std::set<std::string>& clients_in_club) {
...
void handle_client_event_3 (const client_info& client, std::map<int, bool>& table_availability, std::queue<std::string>& clients_waiting_for_tables,
                            std::set<std::string>& clients_in_club, int total_number_of_tables) {
    std::cout << client.time << " " << client.command << " " << client.client_name << "\n";
    if (clients_in_club.count(client.client_name)) {
